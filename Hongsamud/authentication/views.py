...
from Hongsamud import settings  # type: ignore
...
def signup(request):
    if request.user.is_authenticated: #ถ้า user login อยู่แล้ว จะโดนโยนไปหน้า xxx
    ...
    if request.method == 'POST':
        ...
        if User.objects.filter(username=name):
            messages.error(request,"name already exist! Plase try some either")
        ...
        if pass1!=pass2:
            ...
            return redirect('signup')
        if len(pass1)<8:
            messages.error(request,"Passwords so short")
            return redirect('signup')
        ...
        #email
        subject="Welcom to Hallucinated Library"
...
def signin(request):
    if request.method == 'POST':
        Email=request.POST['Email']
        pass1=request.POST['pass1']
        remem=request.POST.get('remember',False)
        try:
